...
- **Статус:** ⏭ — гейт в долге (риск принят, см. «Долг» набора); полноту судили автор и рецензент
- Новые публичные контракты плана: заголовок `Idempotency-Key` и код `200` на повторе → claim-1,
  claim-4; `409` на конфликте → claim-6
