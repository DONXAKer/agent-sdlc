...
Запуск: python3 flow-verdict.py [--at verify|handoff] <корень проекта> <slug>
  --at handoff (умолчание) — полный контракт завершённого витка;
  --at verify — середина витка, этап 6: без проверок handoff'а (приёмка, записи о дефектах) —
                их артефакты ещё законно не существуют.
Код возврата: 0 — контракт цел; 1 — есть провалы; 2 — неверный вызов.
"""
...
def main(root: Path, slug: str, at: str = "handoff") -> int:
    v = Verdict()
    full = at == "handoff"   # False = середина витка (этап 6): handoff-проверки пропускаются
    sdlc = root / ".sdlc"
    d = sdlc / slug
...
if __name__ == "__main__":
    args = sys.argv[1:]
    at = "handoff"
    if args[:1] == ["--at"] and len(args) >= 2:
        at, args = args[1], args[2:]
    if at not in ("verify", "handoff") or len(args) != 2:
        print(__doc__)
        sys.exit(2)
    sys.exit(main(Path(args[0]), args[1], at))
